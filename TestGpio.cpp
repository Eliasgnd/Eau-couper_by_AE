    ...
    : QWidget(parent), ui(new Ui::TestGpio)
{
    ...
    // Connexion du signal SPI pour affichage
    ...
    connect(&updateTimer, &QTimer::timeout,
            this, &TestGpio::updatePinStates);
    updateTimer.start(500);

    connect(ui->buttonMenu, &QPushButton::clicked,
            this, &TestGpio::goToMainWindow);
}
...
void TestGpio::init_gpio()
{
    ...
    for (int pin : outputPins) {
        ...
        if (line &&
            gpiod_line_request_output_flags(line, "testgpio", 0, 0) == 0) {
    ...
    for (int pin : inputPins) {
        gpiod_line *line = gpiod_chip_get_line(chip, pin);
        if (line &&
            gpiod_line_request_input_flags(line, "testgpio",
                                           GPIOD_LINE_REQUEST_FLAG_BIAS_DISABLE) == 0) {
