...
{
    if (m_decoupeEnCours || m_optimizationRunning) {
        qDebug() << "[DEBUG] Découpe déjà en cours dans optimizePlacementComplex";
        QMessageBox* msg = new QMessageBox(QMessageBox::Warning,
                                           "Découpe en cours",
                                           "Impossible de modifier les paramètres ou la forme pendant la découpe.",
                                           QMessageBox::Ok,
    ...
    if (m_isCustomMode && !m_customShapes.isEmpty()) {
        for (const QPolygonF &poly : m_customShapes)
            basePath.addPolygon(poly);
        QRectF customBounds = basePath.boundingRect();
        double scaleX = (customBounds.width() > 0) ? (adaptedLargeur / customBounds.width()) : 1.0;
        double scaleY = (customBounds.height() > 0) ? (adaptedLongueur / customBounds.height()) : 1.0;
        QTransform scaleTransform;
        scaleTransform.scale(scaleX, scaleY);
        basePath = scaleTransform.map(basePath);
    } else {
        QList<QGraphicsItem*> shapesList = ShapeModel::generateShapes(currentModel, adaptedLargeur, adaptedLongueur);
        if (shapesList.isEmpty()) {
            m_optimizationRunning = false;
        ...
        if (auto pathItem = dynamic_cast<QGraphicsPathItem*>(prototype))
            ...
            basePath.addRect(rectItem->rect());
        else if (auto ellipseItem = dynamic_cast<QGraphicsEllipseItem*>(prototype))
            basePath.addEllipse(ellipseItem->rect());
        else if (auto polyItem = dynamic_cast<QGraphicsPolygonItem*>(prototype))
            basePath.addPolygon(polyItem->polygon());
        else {
            progressBar->setVisible(false);
            m_optimizationRunning = false;
            return;
    ...
    QRectF bounds = basePath.boundingRect();
    QTransform normTransform;
    normTransform.translate(-bounds.x(), -bounds.y());
    basePath = normTransform.map(basePath);
    basePath.closeSubpath();

    // Préparer la version tournée à 90°
    QTransform rot90;
    rot90.rotate(90);
    QPainterPath rotatedPath = rot90.map(basePath);
    rotatedPath.closeSubpath();

    QSizeF size0 = basePath.boundingRect().size();
    QSizeF size90 = rotatedPath.boundingRect().size();

    struct FreeRect { QRectF rect; };
    struct PathInfo { QPainterPath path; QRectF bbox; };

    QList<PathInfo> bestPlaced;
    double bestWaste = std::numeric_limits<double>::max();

    int attempts = 5;
    for (int a = 0; a < attempts; ++a) {
        QList<FreeRect> freeRects;
        freeRects.append({QRectF(0, 0, drawingWidth, drawingHeight)});
        QList<PathInfo> placed;
        ...
        for (int n = 0; n < shapeCount; ++n) {
            ...
            for (int i = 0; i < freeRects.size(); ++i) {
                const QRectF &fr = freeRects[i].rect;
                auto tryRect = [&](const QSizeF &s, bool rot){
                    if (s.width() <= fr.width() && s.height() <= fr.height()) {
                        double score = fr.width() * fr.height() - s.width() * s.height();
                        if (score < bestScore) {
                            bestScore = score; bestIdx = i; useRot = rot; }
                    }
                };
                tryRect(size0, false);
                tryRect(size90, true);
            ...
            QRectF fr = freeRects.takeAt(bestIdx).rect;
            QSizeF chosenSize = useRot ? size90 : size0;
            QPainterPath path = useRot ? rotatedPath : basePath;

            QTransform place;
            place.translate(fr.x(), fr.y());
            path = place.map(path);
            QRectF bbox = path.boundingRect();

            placed.append({path, bbox});
            placedPaths.append(path);

            QRectF right(fr.x() + chosenSize.width(), fr.y(), fr.width() - chosenSize.width(), chosenSize.height());
            QRectF bottom(fr.x(), fr.y() + chosenSize.height(), fr.width(), fr.height() - chosenSize.height());
            if (right.width() > 1 && right.height() > 1)
                freeRects.append({right});
            if (bottom.width() > 1 && bottom.height() > 1)
                freeRects.append({bottom});
        }

        double waste = evaluateWasteArea(placedPaths, drawingWidth, drawingHeight);
        if (waste < bestWaste) {
            bestWaste = waste;
            bestPlaced = placed;
        }
    ...
    scene->clear();
    for (const PathInfo &pi : bestPlaced) {
        QGraphicsPathItem *item = new QGraphicsPathItem(pi.path);
        item->setPen(QPen(Qt::black, 1));
        item->setBrush(Qt::NoBrush);
        item->setFlag(QGraphicsItem::ItemIsMovable, true);
        item->setFlag(QGraphicsItem::ItemIsSelectable, true);
    ...
    emit shapesPlacedCount(bestPlaced.size());
    m_optimizationRunning = false;
    progressBar->setVisible(false);
}
