...
Pure business logic with no Qt dependencies where possible.

* shapes/ → ShapeModel, ShapeManager, PathGenerator
* geometry/ → GeometryUtils, PlacementOptimizer
...
* CustomDrawArea (main widget)
* DrawModeManager (state)
* HistoryManager (undo/redo)
* MouseInteractionHandler (input handling)

### ui/

Qt Widgets and dialogs (.ui files via Qt Designer).

* ShapeVisualization (main display)
* Virtual keyboards for touchscreen

### managers/

Application-level services.

* navigation/ → AppController, NavigationController
* system/ → MotorControl, Raspberry (GPIO), WiFi
* ai/ → OpenAIService, AIServiceManager

### models/

Inventory CRUD services.

### infrastructure/persistence/

Data access layer (InventoryRepository).

### viewmodels/

Centralized state (WorkspaceModel).
